function results=CreateAndTrainANN(x,t)
    ...
    if ~isempty(x)
        ...
        E = perform(net,t,y);
% Mean Absolute Error
        MAE = sum(abs(e)) / size(e,2);
        % Mean Predictive Accuracy (MPA)
        %sinja=size(y,2);
        Jam = 0;
        for iinja=1:176
            tinja = t(1,tr.trainInd);
            yinja = y(:,tr.trainInd);
            [ainja, binja] = min(tinja);
            if ainja == 0
                tinja(1, binja) = 1;
            end
            [ainja, binja] = min(yinja);
            if ainja == 0
                yinja(:, binja) = 1;
            end           
            MPA=100*abs((1-(abs(yinja(1,iinja)-tinja(1, iinja))/tinja(1, iinja))));        
            Jam = Jam + MPA;
        end
        MPATrain = Jam/176;
        ...
        Jam = 0;
        for iinja=1:76
            tinja = t(:,[tr.testInd tr.valInd]);
            yinja = y(:,[tr.testInd tr.valInd]);
            [ainja, binja] = min(tinja);
            if ainja == 0
                tinja(1, binja) = 1;
            end
    ...
    Data.e=e;
    Data.E=E;
    Data.MPATrain = MPATrain;
    Data.MPATest = MPATest;
    Data.MAE = MAE;
